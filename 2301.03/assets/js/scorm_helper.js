...
// answer -> result answer every interactivity in game (0 incorrect, 1 correct)
// question -> response answer every interactivity in game
ScormHelper.prototype.pushAnswer = function(answer,response) {
	...
	// localStorage.setItem("cmi.suspend_data", JSON.stringify($this.ldata));
	// $this.sendServer("cmi.suspend_data",JSON.stringify($this.ldata));
	$this.scorm.set("cmi.suspend_data", JSON.stringify($this.ldata));
};