...
Modul.prototype.init = function(current_settings) {
    ...
    if($this.game_data["slide"] == undefined){
        $this.game_data["slide"] = $this.current_settings["slide"];
    }else{
        if($this.game_data["slide"] != $this.current_settings["slide"]){
            $this.game_data["total_soal_current_slide"] = 0;
    ...
    $this.curr_soal = $this.game_data['curr_soal'] != undefined ? $this.game_data['curr_soal'] : 0;
    $this.curr_step = (game.game_data["curr_step"] != undefined ? game.game_data["curr_step"] : 0);
    $this.last_score = (game.game_data["last_score"] != undefined ? game.game_data["last_score"] : 0);
    /*end game data*/
    ...
    $.get("config/setting_quiz_slide_"+$this.current_settings["slide"]+".json",function(e){
        ...
        // $(".slider-content").css("background-size", "cover");
        // $(".slider-content").css("background-repeat", "no-repeat");
        // $(".slider-content").css("background-position", "bottom left");
...
Modul.prototype.mulai_game = function(){
    ...
    if(ldata == undefined || ldata["answer"] == undefined || ldata["answer"]== null || ldata["answer"].length < $this.question_data.length){
        ...
        if(ldata["answer"].length == ldata["list_question"].length){
            ...
            if($this.category_game == ($this.question_datas.length - 1)){
                $this.nextSoalAtLast();
            }else{
                $this.nextSoal();
            }
        ...
        // console.log($this.question_datas);
        // $(".btn-close").hide(); 
        // if(ldata['answer'].length == $this.question_datas.length){
...
Modul.prototype.showQuestion = function() {
    ...
    if($this.ldata2['type'] == 'mc'){
        ...
        $this.ldata2["first_img_character"] = undefined;
...
Modul.prototype.setCurrItem = function(curr_item, max_item){
    var $this = this;
    // alert(max_item);
...
Modul.prototype.settingDialogFeedback = function($clone, benar){
    var $this = this;
    //click button in dialog feedback
    $($clone).find('.btn_next_dialog-2').unbind().click(function(){
...
Modul.prototype.settingDialogFeedback_2 = function($clone, benar, index){
    var $this = this;
...
Modul.prototype.resetTimer = function(){
    clearInterval(timer_interval);
    $(".timer .text_time").html("--:--");
}

Modul.prototype.setQuizFalseAnswer = function(){
    var last_life = parseInt($this.life);
    $this.setFalseAnswer();
    var arr_temp = [];
...
Modul.prototype.setTimerPercent = function(){
    $this = this;
...
Modul.prototype.playVideo_2 = function(id, type = '', callback) {
    $("#video").show();
...
Modul.prototype.playVideo_plyr = function(video,type, callback) {
...
Modul.prototype.stopVideo = function(id, type = '') {
    $("#video").hide();
...
Modul.prototype.stopVideo_plyr = function(video = '') {
    $("#video").hide();
...
Modul.prototype.showModalFeedback = function(benar, timeout = 0){
    var $this = this;
...
Modul.prototype.showModalFeedback_2 = function(benar, timeout = 0){
    var $this = this;
    $(".modal#modal_feedback").find(".title_feedback_2").hide();
    console.log($(".modal#modal_feedback .modal_feedback_black"));
    // $(".modal#modal_feedback .modal_feedback_black").show();
...
Modul.prototype.close_feedback = function(benar, timeout = 0){
    var $this = this;
...
Modul.prototype.close_feedback_2 = function(benar, timeout = 0){
...
Modul.prototype.resetCurrGame = function(){
    var $this = this;
    game.scorm_helper.resetQuizDataByIndex("game_slide_"+$this.current_settings["slide"]+"_"+$this.category_game);
    $this.resetGameData();
    game.game_data["category_game"] = $this.category_game;
...
Modul.prototype.hideItemSelected = function(item_id){
    var $this = this;
    var item_div_n = $this.category_game + 1;
    $('#item_div-'+item_div_n+' #item-'+item_id).hide();
}

Modul.prototype.nextGamePickPlaceItem = function(){
    var $this = this;
    console.log(video1);
    // $this.stopVideo_plyr(video1);
}

Modul.prototype.setScore = function(score){
    var $this = this;
    // var total = game.scorm_helper.getQuizResult(["game_slide_"+$this.current_settings["slide"]+"_"+$this.category_game]);
    // console.log(total);
...
Modul.prototype.showVideoFeedback_2 = function(id_video_before, id_video, video, callback){
    var $this = this;
    // $this.stopVideo(id_video_before);
    $this.opacityVideo(0, id_video, 'feedback');
    // $(".star-wrapper").hide();
...
Modul.prototype.showVideoFeedback_plyr = function(id_video_before, id_video, video, callback){
...
Modul.prototype.resetGameData = function(){
...
Modul.prototype.playBacksound = function() {
    var $this = this;
...
Modul.prototype.playDialogSound = function(src) {
    var $this = this;
...
Modul.prototype.showContent = function(index, $clone = ''){
    var $this = this;
    console.log(index);
    console.log($clone);
    ...
    if($clone == ''){
        ...
        if(index_dialog != undefined && arr_dialog.length > (index+1)){
            ...
            var $label_img = $this.ldata2["text_3"][index]["text_2_img"];
            ...
            // if(arr_dialog.length == 1){
            //     if(arr_img_character != undefined){
            //         if(arr_img_character.length > 0){
                        $($clone).find('.img_item').hide();
                        if(arr_img_character.length == 1){
                            console.log(arr_img_character[0]);
                            $($clone).find('#img_item-'+arr_img_character[0]).css('position','unset');
                            $($clone).find('#img_item-'+arr_img_character[0]).show();
                        }else if(arr_img_character.length == 2){
                            $($clone).find('#img_item-'+arr_img_character[0]).addClass('cust');
            ...
            /*Add text to div-question*/
            console.log(index_dialog);
            $($clone).find(".pilihan_place .txt_question").html(index_dialog["text"]);
            $($clone).find(".pilihan_place .row_pilihan").hide();

            //setting btn_dialog_wrapper
            if(index_dialog["text"] != undefined && index_dialog["text"] != ""){
                console.log($(".btn_dialog_wrapper #btn_dialog-1"));
                $($clone).find(".btn_dialog_wrapper #btn_dialog-1").show();
            }else{
                $($clone).find(".btn_dialog_wrapper .btn_dialog").hide();
            }
            /*End add text to div-question*/

            /*Setting label name question*/
                let $label_text = index_dialog["text_2"];
                let $label_style = index_dialog["label_name_style"];
                let $label_position = index_dialog["text_2_position"]; 
                let $suara_batin = index_dialog["suara_batin"];
                let $label_icon = index_dialog["label_name_icon"];
                let $label_img = index_dialog["text_2_img"];
            ...
            let $image = index_dialog["hand_touch_img"];
            if($image != undefined && $image != ""){
                $this.setHandTouchImg($image);
            }else{
                $(".img_click").hide()
            ...
            // console.log($($clone).find(".btn_show_soal"));
            $($clone).find('.btn_show_soal').show();
            $($clone).find('.btn_next_dialog').hide();

            $($clone).find(".btn_show_soal").unbind().click(function(){
                console.log("btn_show_soal");
                $(this).hide();
...
Modul.prototype.setLabelNameQuestion = function($clone, $label_text, $label_style, $label_position, $suara_batin, $label_icon, $label_img){
...
Modul.prototype.setLabelNameDialog = function($clone, $name_character, $label_style, $label_position, $suara_batin, $label_icon, $label_img, $label_bg_img = ""){
    var $this = this;
...
Modul.prototype.setHandTouchImg = function($image){
...
//function set show or hide dialog feedback base on slide-content id
Modul.prototype.showDialogFeedback = function(index, benar, $clone = ''){
    var $this = this;
        ...
        if(arr_dialog != undefined && arr_dialog.length > 0){
            ...
            if(index_dialog != undefined){
                ...
                if(arr_img_character != undefined){
                    ...
                    //     //if mode suara batin
                    //     // console.log($($clone).find('.div_name_label .name_label_text'));
                    //     // console.log($($clone).find('.div_name_label .name_label_text .img_suara_batin'));
                    //     // console.log($('.img_suara_batin'));
                    //     $($clone).find('.div_name_label .name_label_text .img_suara_batin').remove();
                    //     // console.log(arr_dialog);
                    //     // console.log(index);
                    //     // console.log(arr_dialog[index]['suara_batin']);
                    //     if(arr_dialog[index]['suara_batin'] != undefined){
                    //         if(arr_dialog[index]['suara_batin'] == 1){
                    //             var img_html = '<img class="img_suara_batin" src="assets/image/other/thinking_emoji.png" style="width: 6.4vw;margin-left: 1.4vw;">';
                    //             $($clone).find('.div_name_label .name_label_text').append(img_html);
                    //         }
                    //     }
                    ...
                    //             $($clone).find('.div_name_label .div_name_label-2').css("float",arr_dialog[index]['text_2_position']);
                    //             $($clone).find('.div_name_label').addClass(arr_dialog[index]['text_2_position'].toLowerCase()+"_label");
                    //         }else{
                    ...
                    //         $($clone).find('.div_name_label-2 .name_label_text').removeAttr("style");
                    //         console.log(arr_dialog);
                    //         console.log($this.ldata2["label_name_img"]);
                    //         console.log(arr_dialog[index]['label_name_img']);
                    //         if(arr_dialog[index]["label_name_style"] != undefined){
                    //             $($clone).find('.div_name_label-2 .img_label').hide();
                    //             $($clone).find('.div_name_label-2 .name_label_text').css(arr_dialog[index]["label_name_style"]);
                    //             // $($clone).find('.div_name_label-2 .name_label_text').css("padding","0.4vh 4.3vw");
                    //         }else if(arr_dialog[index]['label_name_img'] != undefined && arr_dialog[index]['label_name_img'] != ""){
                    //             $($clone).find('.div_name_label-2 .img_label').show();
                    //             $($clone).find('.div_name_label-2 .img_label').attr("src","assets/image/other/"+arr_dialog[index]['label_name_img']);
                    //         }else if($this.ldata2["label_name_img"] != undefined && $this.ldata2["label_name_img"] != ""){
                    //             console.log("test");
                    //             console.log($($clone).find('.div_name_label-2 .img_label'));
                    //             $($clone).find('.div_name_label-2 .img_label').show();
...
Modul.prototype.setTextButton = function(class_name, text){
    if(text == 'Try Again'){
...
Modul.prototype.showHideSoal = function(mode){
    if(mode == 'show'){
        // $('.modal-backdrop-cust').show();
...
//stop backsound
Modul.prototype.stopBackSound = function(){
    var $this = this;
    //pause sound
    $this.audio_dynamic_2.pause();
    $this.audio_dynamic_2.currentTime = 0;
...
Modul.prototype.substringText = function(string, max_string){
    var $this = this;
...
Modul.prototype.substringText_2 = function(string, max_string){
/*Function set dialog text*/
    let dialog_text = string;
    console.log(dialog_text.length);
    console.log(max_string);
...
Modul.prototype.stripHtml = function (html){
...
Modul.prototype.getHtmltag = function (a){
...
Modul.prototype.showQuestion_2 = function($clone){
    var $this = this;
    $this.showHideSoal('show');
    $($clone).find('.div_name_label').hide();
    $($clone).find('.img_item').hide();
...
// Modul.prototype.setProgresBar = function(mode = '') {
//     var $this = this;
...
Modul.prototype.nextDialog = function($element) {
    var $this = this;
    console.log($element);
...
Modul.prototype.nextSoal = function(){
    var $this = this;
    $this.category_game += 1; 
    $this.game_data['category_game'] = $this.category_game;
    // console.log(game.game_data);
...
Modul.prototype.nextSoalAtLast = function(){
    var $this = this;
    ...
    /*Function set stage clear*/ 
        ...
        if($this.game_data["total_answer_true"] == $this.game_data["total_soal_current_slide"]){
            complete_stage.push($this.curr_step);
        }else{
            failed_stage.push($this.curr_step);
        }
