...
Slider.prototype.createSlider = function() {
		...
		if($this.listSlider[i]["button"]){
			for (var j = 0; j < $this.listSlider[i]["button"].length; j++) {
				...
				if($this.listSlider[i]["button"][j]["gotoSlide"]){
					...
					$(cloneBtn).click(function(e){
						...
						// pause sound
				    	// if($this.audio_dynamic != undefined){
					    // 	$this.audio_dynamic.pause();
					    //     $this.audio_dynamic.currentTime = 0;
					    // }
         ...
         //                $this.audio_dynamic_2.currentTime = 0;
					...
					if($this.listSlider[i]["video"]){
						...
						$(cloneBtn).unbind().click(function(e){
							// $(this).off();
						    ...
						    $('.close-modal').unbind().click(function(e){
						    	$('.popupdialogbox #confirm_1').hide();
						        $('.popupdialogbox').modal('hide');
						    	game.setSlide(3);
						    });

						    $('.btn-standard--no').unbind().click(function(e){
						    	$('.popupdialogbox #confirm_1').hide();
						        $('.popupdialogbox').modal('hide');
						        game.audio.audioButton.play();
						    });
    ...
    $("#ulasan").on('afterChange', function(event, slick, currentSlide, nextSlide){
     ...
     //    var src_audio = "assets/audio/"+$this.listSlider[id]['sound'];
     //    // game.audio_dynamis(src_audio);

     //    $this.audio_dynamic = game.audio.audio_dynamic(src_audio);
     //    $this.audio_dynamic.play();
     });
    ...
    // var src_audio = "assets/audio/"+$this.listSlider[0]['sound'];
    // $this.audio_dynamic = game.audio.audio_dynamic(src_audio);
    // $this.audio_dynamic.play();
...
Slider.prototype.playBacksound = function() {
    var $this = this;
    if($this.backsound != undefined){
        if($this.backsound == 1){
            //play sound 2
            var src_audio_2 = "assets/audio/Autumn-day-easy-listening-music.mp3";
