var GameMapStage = function(){
   
}
   
GameMapStage.prototype.init = function(current_settings) {
    console.log("GameMapStage");
    var $this = this;
    ...
    $this.unlock_all_stage = true;
    $this.slide_game_map = game.slide_game_map
    ...
    $.get("config/setting_quiz_slide_"+$this.current_settings["slide"]+".json",function(e){
        ...
        $.get("config/setting_game_map_stage.json",function(e2){
          console.log(e2);
  ...
  /*Function get total soal from all stage*/
GameMapStage.prototype.get_total_soal = function() {
 ...
 GameMapStage.prototype.getQuestion_ulartangga = function() {
    var $this = this;
    var arr_quest = [];
    var arr_rand = [];
    var other = [];
...
//START GAME
GameMapStage.prototype.mulai_game_ulartangga = function() {
  var $this = this;
...
GameMapStage.prototype.setButton_ulartangga = function() {
  var $this = this;

  if($this.unlock_all_stage == true){
      $(".button").removeClass("disabled");
      $(".button").addClass("active");

      var game_quiz = game.scorm_helper.getQuizResultPrefix();
      console.log(game_quiz);
      var total_stage_played = 0;
      ...
      $(".button").click(function(e){
        ...
        if($this.curr_step != $this.total_step){
          ...
          if($this.total_step != total_stage_played){
              game.setSlide(($this.slide_game_map+parseInt(id)));
          }else{
              game.nextSlide();
          }
        }
    ...
    if($this.curr_soal > $this.total_step){
      ...
      $(".btn-finish, .girl").click(function(e){
...
GameMapStage.prototype.show_question_ulartangga = function() {
  console.log("show_question_ulartangga");
   ...
   $clone.parent().find(".header_quiz").find("img").attr("src","assets/image/game_map_stage/"+$this.list_logo_quiz[$this.count_soal]);
   ...
   if($this.question_data[$this.list_soal[$this.curr_soal]]["type"] == "dad"){
     ...
     if($this.question_data[$this.list_soal[$this.curr_soal]]["image"]){
      ...
      $clone.find(".row.image").attr("src","assets/image/game_map_stage/quiz-image/"+$this.question_data[$this.list_soal[$this.curr_soal]]["image"]);    
     }else{
...
GameMapStage.prototype.initDad = function(slider_content) {
  var $this = this;
  var start=0;
  var width=0;
...
GameMapStage.prototype.setEvent = function($clone) {
  var $this = this;
...
GameMapStage.prototype.runSlick = function(elem) {
  elem.slick({
      dots: true,
      infinite: false,
      speed: 500
...
GameMapStage.prototype.prev = function(prev) {
   var $this = this;
   if(prev){
...
GameMapStage.prototype.next = function() {
   var $this = this;
...
GameMapStage.prototype.cek_jawaban_ulartangga = function($clone,$type) {
   var $this = this;
   var $flag=0;
   var count = 0;
...
GameMapStage.prototype.setTutorial_ulartangga = function() {
  var $this = this;
  // $this.popup_slick = 1;
...
GameMapStage.prototype.show_life = function() {
...
GameMapStage.prototype.setProgresBar = function() {
  var $this = this;
  console.log($this.curr_step);
  console.log($this.total_step);
...
GameMapStage.prototype.showModal = function() {
...
GameMapStage.prototype.showStep = function(){
  var $this = this;
  ...
  //generate start step
  // $(start_step_clone).find(".idle").attr("src","assets/image/game_map_stage/"+$this.start_step["image"]);
  ...
  // $(finish_step_clone).find(".idle").attr("src","assets/image/game_map_stage/"+$this.finish_step["image"]);
  ...
  for (var i = 0; i < $this.step_data.length; i++) {
      ...
      $(clone_step_2).attr("index",(i+1));
      if(i != 0){
        $(clone_step_2).addClass("disabled");
      }
      $(clone_step_2).find(".idle").attr("src","assets/image/game_map_stage/"+$this.step_data[i]["image"]);
      $(clone_step_2).find(".success").attr("src","assets/image/game_map_stage/"+$this.step_data[i]["image_2"]);
      $(clone_step_2).find(".locked").attr("src","assets/image/game_map_stage/"+$this.step_data[i]["image_3"]);
...
GameMapStage.prototype.sliderPopup = function($elementModal) {
  var $this = this;
