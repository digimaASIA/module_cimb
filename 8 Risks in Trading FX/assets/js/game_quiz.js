...
GameQuiz.prototype.init = function(current_settings) {
	...
	$this.animation_card_timeout = 2000;
...
GameQuiz.prototype.show_question = function() {
	...
	// get current soal
	$clone.css("background",$this.background);	
		...
		$current_soal["type"] == "mc_custom")
	{
		...
		if($current_soal["type"] != "tf"){
			...
			for (var i = 0; i < arr_rand.length; i++) {
				...
				$app_pilihan.find(".txt_pilihan").html("<p>"+$current_soal["pilihan"][arr_rand[i]]["text"]+"</p>");
				...
				/*Setting css mc_custom*/
				if($current_soal["type"] == "mc_custom"){
					$clone.find(".pilihan_wrapper").addClass("flex-container");
					$clone.find(".card_wrapper").show();
					$($app_pilihan).find(".bullet").hide();
					$($app_pilihan).addClass("col-xs-6");
					$clone.find(".text_question").hide();
					$this.initCardSorting($clone);
				}
				/*End setting css mc_custom*/
...
GameQuiz.prototype.initCardSorting = function(slider_content) {
	var $this = this;
	...
	for (var i = $this.list_question.length-1; i >=0 ; i--) {
		...
		if(i>=$this.curr_soal){
			...
			$($clone).find(".desc_card").html($this.question_data[$this.list_question[i]]["question"]);
			// console.log($clone);
		...
		// 	if(parseInt($this.arr_data["answer"][($this.list_question.length-1)-i]) == 1){
...
GameQuiz.prototype.settingPage = function($clone) {
	...
	if($this.type == "page"){
		...
		$($clone).find(".next-soal").click(function(e){
			...
			var curr_card = $($clone).attr("curr_soal");
			// console.log(curr_card);
			$('#move-'+curr_card).removeClass('two-a');
			$('#move-'+curr_card).addClass('two-b');
			setTimeout(function(){
				$this.next();
			},$this.animation_card_timeout);
		});
	}
	...
	else if($current_soal["type"] == "mc" || $current_soal["type"] == "tf_custom" || $current_soal["type"] == "mc_custom"){
		$clone.find(".btn-submit").hide();
		$clone.find(".next-soal").show();
		$clone.find(".next-soal").attr("disabled","disabled");
