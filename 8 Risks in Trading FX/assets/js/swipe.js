...
Swipe.prototype.init = function(current_settings) {
	...
	$this.alert_jwb = $(".question_wrapper").find(".alert_jawab").first().clone();
	$this.btn_submit = $(".question_wrapper").find(".btn_submit").first().clone();
...
Swipe.prototype.appendSlide = function() {
	...
	$clone.find(".building img").attr("src","assets/image/swipe/building/"+$this.history[$this.curr_his]["build"]);
	...
	if(!$this.first){
		if($this.history[$this.curr_his]["is_box"] && game.scorm_helper.getLengthListAns()<$this.history[$this.curr_his]["count"]){
			$(".img-people").attr("src","assets/image/swipe/people/MC_CIMB_J05_C03_M03_01b_v.0.0.1.gif");
			$(".pnjk_swipe").hide();
		}
...
Swipe.prototype.init_soal = function(idx) {
	var $this = this;
...
Swipe.prototype.check_complete = function() {
	var arr_jwb = [];
	var arr_quest = [];
	var is_jawab = true;
	var $this = this;
